# Scaffolded Question(s) for Jasmine Bailey
**Scaffolding Level:** moderate | **Mode:** Teacher Edition (includes answer hints) | **Reading Level:** Grade 3 (iReady)

## Scaffolding Rationale
Jasmine's IEP benchmarks include: accurately annotating text, answering literal comprehension questions, writing claims with evidence, and finding textual evidence. Scaffolds should BUILD these skills, not bypass them.

Key considerations:
- She can decode grade-level words but struggles with literal and inferential comprehension
- She benefits from graphic organizers and checklists (starting to find value in them)
- Writing is stronger with 1:1 support; independent writing is ~50% accuracy
- She needs to use reference sheets and class notes more effectively (Math benchmark 3, applicable to ELA too)

## DR-1A: Think & Share
**Original Question:** What claim does Lowe make about the word 'community' in paragraph 1?
**Paragraph(s):** 1-2

### Scaffolded Version (moderate)

**Modified question for Jasmine:**

Read paragraph 1. Lowe says the word 'community' has a 'strange power.'
What does he mean? Choose one:
- [ ] People use the word a lot but don't explain what they mean
- [ ] Community is a dangerous word
- [ ] Nobody talks about community
Now tell your partner: Why do you think people don't say what they mean by 'community'?
---

## DR-1B: Write (Optional)
**Original Question:** What is the relationship between the claim above and the bulleted list in paragraph 2?
**Paragraph(s):** 1-2

### Scaffolded Version (moderate)

**Modified question for Jasmine:**

Look at the bullet list in paragraph 2.
Count the bullet points. Lowe lists things that a good definition of community should explain.

**Sentence starter:** "In paragraph 2, Lowe lists six things that a definition of community needs to explain. This connects to paragraph 1 because _____."

*Expected direction: The list supports Lowe's claim that people don't define 'community' clearly, by showing what a good definition must cover.*
---

## DR-1C: Turn & Talk
**Original Question:** Based on the bulleted list, summarize at least three key traits of a community.
**Paragraph(s):** 1-2

### Scaffolded Version (moderate)

**Modified question for Jasmine:**

Look at the bullet list in paragraph 2. Find THREE traits of a community.
Fill in:
1. There are many different kinds of communities (like places, religions, or __________).
2. Communities can make people feel __________ and like they belong.
3. Communities shape what people think is __________ and __________.

*Expected answers: (1) interests/shared practices, (2) togetherness/positive, (3) good, bad*
---

## DR-2A: Write
**Original Question:** How does Lowe's example of belonging to the Newcastle community support his definition?
**Paragraph(s):** 3-7

### Scaffolded Version (moderate)

**Modified question for Jasmine:**

Lowe says a community is 'a group of people who share an identity-forming narrative' (paragraph 3).
He uses Newcastle as an example. In paragraphs 5-6, find:
- What does Lowe DO to be part of Newcastle's community? (list 2 things)
- How does being part of Newcastle make him FEEL? (list 1 feeling)

**Sentence starter:** "Lowe's example supports his definition because Newcastle's shared story is part of who he is. For example, he _____ and _____."

*Expected direction: He shows people around the city, feels pride/at home when hearing the accent, attends football matches, adopted Newcastle character traits. These show the community's story is part of his identity.*
---

## DR-2B: Turn & Talk
**Original Question:** Consider the communities you are a part of. Share how one of these communities fits Lowe's definition.
**Paragraph(s):** 3-7

### Scaffolded Version (moderate)

**Modified question for Jasmine:**

Think about a community YOU are part of (your school, neighborhood, team, family, etc.).

Fill in before talking:
- My community: __________
- Our shared story: __________
- Something I do that shows I'm part of this community: __________
---

## DR-3A: Turn & Talk
**Original Question:** Refer back to the bulleted list in Paragraph 2. Why does the author include another bulleted list in this section?
**Paragraph(s):** 8

### Scaffolded Version (moderate)

**Modified question for Jasmine:**

Look at paragraph 2 (the first list) and paragraph 8 (the second list).
Both lists have 6 bullet points.

**Question:** Why did Lowe write TWO lists?
- The first list says what a definition NEEDS to do
- The second list shows that his definition **meets** each of those requirements

In your own words: Why does Lowe go through each trait a second time?

*Expected answer: Lowe tests his definition against the 6 criteria from paragraph 2 to prove it works.*
---

## DR-3B: Write
**Original Question:** In your own words, explain at least three reasons why Lowe thinks his definition that community is 'a shared story' works.
**Paragraph(s):** 8

### Scaffolded Version (moderate)

**Modified question for Jasmine:**

In paragraph 8, Lowe checks his definition against 6 traits.
Choose 3 of these traits and explain in your own words:

| Trait | Why Lowe's definition works |
| --- | --- |
| Different types of communities | A "shared story" can be about a place, religion, or __________ |
| Positive feelings | Being part of a shared story makes people feel __________ |
| Part of many communities | People can have more than one __________ |

*Expected answers: (1) any other social practice/interest, (2) like they belong / part of something larger / togetherness, (3) identity-forming story / shared story*
---

## DR-4: Find Evidence
**Original Question:** The title of this text is 'What is community and why is it important?' Highlight two pieces of evidence that answer both questions.
**Paragraph(s):** 10-11

### Scaffolded Version (moderate)

**Modified question for Jasmine:**

The title asks TWO questions:
1. **What** is community?
2. **Why** is it important?

Find one quote that answers each:
- WHAT is community? Look in paragraphs 9-10 for Lowe's definition. Start with "Community is..."
- WHY is it important? Look in paragraph 11 for why Lowe thinks community matters for "social change."
---

## MC-1: Multiple Choice
**Original Question:** Which best describes the author's main purpose in writing the article? [RI.6]
A. to illustrate how important it is to be part of multiple communities
B. to demonstrate the way community has shaped the author's life
C. to describe the life of a person who lives in Newcastle
D. to give a clear and useful definition of community

### Scaffolded Version (moderate)

**Step 1:** Cross out the answer that is clearly wrong:
- C talks about 'describing life in Newcastle' -- is that what the WHOLE article is about? No. Cross out C.
**Step 2:** Now choose between A, B, and D. The article mostly:
- Talks about being part of multiple communities (A), OR
- Shows how community shaped the author's life (B), OR
- Gives a clear definition of community (D)?
---

## MC-2: Multiple Choice
**Original Question:** Which piece of evidence best illustrates the central idea of the article? [RI.1]
A. "So, one of the communities that I consider myself to be part of is the community based around the city of Newcastle." (Paragraph 5)
B. "It is this choice to participate in the making and remaking of these stories about the city that makes me part of the community of Newcastle." (Paragraph 7)
C. "Community is a group of people who share a story that is so important to them that it defines an aspect of who they are." (Paragraph 9)
D. "Social change requires that we rewrite our communal narratives." (Paragraph 11)

### Scaffolded Version (moderate)

**Step 1:** Read each quote. Ask: Does this quote capture the MAIN IDEA?
**Step 2:** The main idea is Lowe's DEFINITION of community.
Which quote IS the definition? Look for the word 'community is...'
---

## MC-3: Multiple Choice
**Original Question:** What is the best meaning of the word "slighted" as it is used in the excerpt from paragraph 5? [RI.4]
A. bored
B. insulted
C. motivated
D. recognized

### Scaffolded Version (moderate)

**Step 1:** Read the sentence: "I feel slighted when people say horrible things about it."
**Step 2:** People are saying HORRIBLE things about Newcastle. How would that make Lowe feel?
- Bored? Insulted? Motivated? Recognized?
Which feeling makes sense when someone says something mean about a place you love?
---

## MC-4: Multiple Choice
**Original Question:** How do paragraphs 5-7 help the reader understand the author's central idea? [RI.2]
A. by explaining why community is neither good nor bad
B. by describing life for members of the Newcastle community
C. by giving an example of a community that is part of the author's story
D. by illustrating how the Newcastle community has many different members

### Scaffolded Version (moderate)

**Step 1:** Paragraphs 5-7 are about Lowe and Newcastle.
**Step 2:** WHY did Lowe include this story? To:
- Explain why community is neither good nor bad (A)?
- Describe Newcastle life (B)?
- Give a PERSONAL EXAMPLE of his definition (C)?
- Show Newcastle has many members (D)?
Think: Does this example help you understand what 'identity-forming narrative' means?
---

## SA-1: Short Answer
**Original Question:** You have just read "What is community and why is it important?" by Toby Lowe. Explain what Lowe means when he says a community is "a group of people who share an identity-forming narrative." [RI.2]

Make sure to incorporate relevant unit vocabulary in your writing.

Self-Checklist: Did I fully answer the prompt? Did I include at least two details from the text? Did I incorporate relevant unit vocabulary?

### Scaffolded Version (moderate)

**Graphic Organizer for Short Answer:**

| Step | What to Write | Where to Find It |
| --- | --- | --- |
| 1. Claim | What does "identity-forming narrative" mean? | Paragraph 3-4 |
| 2. Evidence #1 | Quote or paraphrase from the text | Paragraph 4 or 9 |
| 3. Explain Evidence #1 | How does this help explain the definition? | Your own words |
| 4. Evidence #2 | Another quote or detail from the text | Paragraphs 5-7 |
| 5. Explain Evidence #2 | How does this connect? | Your own words |

**Sentence starters:**
- Claim: "When Lowe says a community shares an 'identity-forming narrative,' he means that _____."
- Evidence: "For example, Lowe explains that _____." (paragraph ___)
- Analysis: "This shows that an identity-forming narrative is _____ because _____."

**Unit vocabulary to include:** narrative, aspect, moral, specific

*Expected answer direction:*
*- Claim: ...he means that a community shares a story so important it becomes part of who they are.*
*- Evidence #1: "Those people build the shared story archetypes of that community into their sense of themselves" (para 4).*
*- Evidence #2: Lowe shows people around Newcastle, feels pride and at home hearing the accent (paras 5-6).*
*- Analysis: An identity-forming narrative is a story that shapes a person's identity and worldview.*
---

## DISC-1: Student-Led Discussion
**Original Question:** Consider a community you are part of. What is something that is considered courteous behavior in that community?

### Scaffolded Version (moderate)

**Before you talk with your partner, fill in:**
- A community I'm part of: __________
- Something that is courteous (polite/kind) in that community: __________

**Sentence starter:** "In my __________ community, it is courteous to _________. For example, _________."

*Hint for Jasmine: Think about Riverstone Prep. What's something people do here to be kind or respectful?*
---

## DISC-2: Student-Led Discussion
**Original Question:** Do you think it is possible to be a part of a community without conforming to its views? Explain your answer.

### Scaffolded Version (moderate)

**Think about it this way:**
- Can you be part of a team but disagree with some of the rules?
- Can you be part of a family but have different opinions than your family?

**Choose your position:**
- "Yes, I think you can be part of a community without agreeing with everything because _____."
- "No, I think you have to agree because _____."
---

## DISC-3: Student-Led Discussion
**Original Question:** Lowe says that "people can be part of many communities." What might be some of the benefits of belonging to more than one community?

### Scaffolded Version (moderate)

**List 2 communities you belong to:**
1. __________
2. __________

**Now think:** What's one good thing about being part of BOTH?
**Sentence starter:** "One benefit of belonging to more than one community is _________ because _________."
---

