# Lesson Modification Request

## Student Profile
**Name:** Jasmine Bailey
**Grade:** 7th grade
**Disability:** Health Impairment
**Reading Level:** 3rd grade (iReady) | **Informational Text:** Grade 2
**ELA Grade:** 1.8/3.0 (3 = mastery)

### Key Strengths
- Contributes thoughtful comments in class when calm and focused
- Open to feedback, especially during 1-on-1 check-ins
- Warm personality, gets along well with peers
- Likes to participate and help others
- Enjoys having class jobs and responsibilities

### Key Challenges
- Academic frustration leads to avoidance behaviors (head down, bathroom requests)
- Shuts down rather than asking for help when stuck
- Low reading and writing stamina
- Difficulty sustaining attention during whole-group instruction
- Inconsistent effort on writing assignments

### What Motivates Jasmine
- Specific positive praise
- Peer interaction and socialization
- Creative activities (drawing, dance)
- Class jobs and helping others
- 1-on-1 attention from adults

## Relevant IEP Goals
### ELA Goal
**Target:** Jasmine will increase her ability to comprehend complex texts to independently answer literal and inferential comprehension questions or writing prompts that meet grade-level rubrics, from 50% to 75%.
**Baseline:** Jasmine is at a 3rd-grade reading level (iReady Fall 2025). Subcategory: Vocabulary Grade 3, Literature Comprehension Grade 3, Informational Text Grade 2. Current ELA grade is 1.8/3.0. She can partici...
**Benchmarks:**
- Jasmine will be able to accurately annotate a given text for the prompt, labeled annotation focus, supporting detail, etc.
- Jasmine will increase her ability to independently answer literal comprehension questions about a text such as main idea, setting, or plot.
- Jasmine will increase her ability to write a claim that accurately answers each part of the question.
- Jasmine will increase her ability to independently find 3 effective pieces of textual evidence to support the claim and answers.
- Jasmine will increase her ability to write analysis to show how her evidence is relevant to and contributes to her claim.

### Self-Regulation Goal
**Target:** Jasmine will improve her self-regulation skills by using a taught calming strategy (deep breathing, movement break, fidget tool, sensory tool, or grounding exercise) to calm her body and re-engage back to tasks in 4 out of 5 opportunities (80%).

## Required Accommodations (Legally Mandated)
- [presentation] Repeat directions
- [presentation] Reminders to pause, plan, proceed
- [presentation] Copy of teacher's notes
- [presentation] Reference sheets, graphic organizers, and checklists
- [timing] Extra time
- [timing] Frequent breaks
- [timing] Scheduled breaks
- [setting] 1:1 check-ins
- [setting] Reminder to remain engaged
- [setting] Small group (as needed)
- [setting] Sit in the front of the room

## Modifications
- [content] Multimodal instruction
- [instruction] Inclusion support
- [instruction] Small group pull-outs when appropriate

## Lesson Context
**Title:** What is 'Community' and why is it important?
**Unit:** Unit 1: Community and Belonging (Lesson 1 of 8)
**Standard:** RI.7.2
**Skill Focus:** Determining and summarizing the central idea of a text and identifying the details that develop it
**Total Duration:** 45 minutes
**Unit Importance:** This is the foundational lesson. The community definition introduced here is referenced throughout the remaining 7 lessons. If the student does not grasp this definition, subsequent lessons will be significantly harder.

## All Activities
### Intro Slide Deck (5 minutes, whole-class)
Guide students through the slide deck to introduce them to the reading lesson and skill focus.
Content: Purpose for Reading: To understand what a community is so that we can build our understanding about how communities create feelings of both belonging and rejection.

### During Reading Questions (15 minutes, whole-class)
Students read and answer the During Reading Questions. Uses whole class, partner, and whole class reading modalities across different paragraph sections.
Content: Paragraphs 1-2 (Whole Class): Think & Share, Write, Turn & Talk about community definition and key traits. Paragraphs 3-7 (Partner): Write about Newcastle example, Turn & Talk about personal communities. Paragraph 8 (Whole Class): Turn & Talk about bulleted list structure, Write three reasons definition works. Paragraphs 10-11: Find Evidence answering title question.

### Independent Practice (20 minutes, independent)
Students complete 4 multiple choice questions and a short answer prompt to measure their understanding of the text.
Content: 4 MC questions testing author's purpose [RI.6], central idea evidence [RI.1], vocabulary in context [RI.4], and paragraph function [RI.2]. 1 short answer: Explain what Lowe means by 'identity-forming narrative' with at least two text details and unit vocabulary.

### Student-Led Discussion (5 minutes, discussion)
Students practice speaking with academic language as they discuss questions about the text with a partner.
Content: 3 discussion questions: (1) Courteous behavior in your community, (2) Can you be part of a community without conforming to its views? (3) Benefits of belonging to more than one community.

## Requested Focus Areas
- **Engagement (UDL Principle 1):** How to recruit and sustain Jasmine's interest, especially when she encounters frustration with grade-level text
- **Representation (UDL Principle 2):** How to present the content accessibly given her 3rd-grade reading level and Grade 2 informational text comprehension
- **Action & Expression (UDL Principle 3):** Alternative ways for Jasmine to demonstrate understanding beyond open-ended writing
- **Accommodation Compliance:** Specific implementation of each legally mandated accommodation during this activity
- **Self-Regulation Support:** Proactive strategies to prevent shutdown/avoidance and support Jasmine's self-regulation IEP goal

## Instructions for Generating Modifications
Generate SPECIFIC, ACTIONABLE modifications that a teacher can use without further editing.
Every modification must be grounded in BOTH the lesson content AND the IEP.
Be specific about:
- Which paragraph/question/activity moment the modification applies to
- Exact wording of scaffolds, sentence starters, or prompts
- How to implement each accommodation during this activity
- Proactive self-regulation check-in points (not just reactive)

AVOID generic advice like 'provide visual supports' or 'use graphic organizers.'
Instead, describe THE SPECIFIC graphic organizer with its content pre-filled from this lesson.